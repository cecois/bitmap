	...
	renderController: function() {
		...
		case "nominatum":
			...
			// if (rcresp.coordinates == 999) { //just a way to fake failure
				addGeoJSONPoly(rcresp);
				appActivityView.reset();
				appConsole.set({"message":"geocoding/data courtesy of OSM &amp; <a href='http://wiki.openstreetmap.org/wiki/Nominatim'>MapQuest/Nominatum</a>"});				
			} //poly.geojson defined?
			else {
				var q = '../../geonames/collection1/select?q=name:"' + qt + '"*&wt=json&sort=score%20desc,population%20desc&rows=1';
				// var q = 'http://localhost:8080/geonames/collection1/select?q=name:"' + qt + '"*&wt=json&sort=score%20desc,population%20desc&rows=1'; //dev
				this.model.set({
					url: q,
					source: "geonames"
				});
				// var rcresp = this.renderGeoNames();
			}
			...
			// appActivityView.reset();
			// 	appConsole.set({"message":"geocoding data courtesy of <a href='http://geonames.org'>GeoNames</a>"});
		...
		var gnresp = $.ajax({
			...
			'success': function(data, textStatus) {
				...
				if(json.response.numFound == 0){
					...
					appActivity.set({spin:false,message:msg});
					resp=false
				}
				var lat = json.response.docs[0].latitude;
...
var resp=false;
		var poly = $.ajax({
			...
			'url': this.model.get("url"),
			'beforeSend':function(){
		appActivity.set({spin: true,message: "geocoding with Nominatum..."});
			},
			// 'dataType': "jsonp",
				...
				if (gdata.coordinates.length > 0) {
					...
					// if(gdata.geojson.length > 0){
						// oh, and it had geojson on board!
										resp = gdata;
										console.log("resp at 159:"); console.log(resp);
										return resp
									// }
				}
					...
					{
						console.log("erf, poly will eq false")
						resp=false
						return resp
					}
					return resp;
...
function addGeoJSONPoly(geofrag) {
...
function addGeoJSONPoint(geofrag) {
